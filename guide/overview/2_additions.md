...
// contracts/common/error.schema.json
{
  ...
  "properties": {
    "code": { "enum": ["VALIDATION_ERROR","PRECONDITION_FAILED","RATE_LIMIT","UPSTREAM_TIMEOUT","TRANSIENT_NETWORK","TOOL_FAILURE","MODEL_OUTPUT_INVALID","BUDGET_EXCEEDED","FATAL"] },
    "message": { "type": "string" },
    "details": { "type": "object" }
  }
}
...
// contracts/agents/dev-writer/v1/request.schema.json
{
  ...
  "type": "object",
  "required": ["agent","version","step","input","context","constraints","idempotency_key"],
  "properties": {
    ...
    "constraints": {
      "type": "object",
      "properties": { "budget_tokens": { "type": "integer" }, "timeout_s": { "type": "integer" } }
    },
...
// contracts/agents/dev-writer/v1/step.manifest.json
{
  ...
  "timeout_s": 180,
  "max_retries": 3,
  "backoff": { "type": "exponential", "base_seconds": 2, "max_seconds": 90, "jitter": true },
  "retry_on": ["RATE_LIMIT","UPSTREAM_TIMEOUT","TRANSIENT_NETWORK","TOOL_FAILURE"],
  "non_retry_on": ["VALIDATION_ERROR","PRECONDITION_FAILED","MODEL_OUTPUT_INVALID","BUDGET_EXCEEDED","FATAL"],
  "idempotent": true,
...
### Бюджет времени и тайм-ауты по шагам (feature\_s)

| Шаг    |    p95 цель | timeout\_s | retries | tokens\_budget |
| ------ | ----------: | ---------: | ------: | -------------: |
| plan   |       4 мин |        300 |       2 |             6k |
...
```json
{ "tokens_budget": 20000, "max_output_tokens": 2000, "hard_cost_stop_usd": 0.60 }
```
...
1. **Старт задачи**: Dashboard → New Task → form(repo, feature\_type, caps) → Create → Task view с live-logs.
...
POSTGRES_URL=postgres://user:pass@localhost:5432/quanta
TELEGRAM_BOT_TOKEN=...
...
services:
  db:
    image: postgres:15
    environment: { POSTGRES_DB: quanta, POSTGRES_USER: user, POSTGRES_PASSWORD: pass }
    ports: ["5432:5432"]
...
```json
{
  ...
  "type": "object",
  "required": ["version","project","runtime","orchestrator","models","cost_caps","pipeline"],
  "properties": {
    "version": { "const": "v1" },
    "project": {
      "type": "object",
      ...
      "properties": {
        ...
        "repo_url": { "type": "string", "pattern": "^https://.+\\.git|^https://github\\.com/.+/.+" },
...
execSync('npx ajv validate -s _schemas/quantapilot.schema.json -d .tmp.qp.json -c ajv-formats', {stdio:'inherit'});
```
