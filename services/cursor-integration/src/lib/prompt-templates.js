...
class PromptTemplateManager {
   ...
   */
  constructor(config = {}) {
    this.templatesPath = config.templatesPath || path.join(__dirname, '../templates');
...
Provide comprehensive QA assessment and recommendations.`,
  ...
  async saveTemplatesToDisk(templates) {
    try {
      const templatesFile = path.join(this.templatesPath, 'default-templates.json');
  ...
  renderTemplate(templateId, context = {}) {
    ...
    try {
      const renderedUserPrompt = this.interpolateTemplate(template.userTemplate, context);
      const renderedSystemPrompt = this.interpolateTemplate(template.systemPrompt, context);
  ...
  addTemplate(templateId, template) {
    const requiredFields = ['role', 'category', 'title', 'systemPrompt', 'userTemplate'];
